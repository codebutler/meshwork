...
namespace FileFind.Meshwork.GtkClient {
	...
	public class ZoomableNetworkMap : ZoomableCairoArea
	{
		...
		private static readonly double AvatarInscribedCircleDiameter = GetNgonInscribedCircleDiameter (4, AvatarDimension);
		private static readonly double NgonSideLength = AvatarInscribedCircleDiameter * 2;
		...
		public ZoomableNetworkMap ()
		{
			...
			Core.NetworkAdded +=
				(NetworkEventHandler)DispatchService.GuiDispatch(
					new NetworkEventHandler(AddNetwork)
				);

			Core.NetworkRemoved +=
				(NetworkEventHandler)DispatchService.GuiDispatch(
					new NetworkEventHandler(DeleteNetwork)
				);
			...
			// this.network.SettingsLoaded += delegate (object sender, EventArgs e) { UpdateNode (network.LocalNode); };
			// this.Events = this.Events | Gdk.EventMask.PointerMotionMask;
			// this.MotionNotifyEvent += new MotionNotifyEventHandler (OnMotionNotifyEventHandler);
		...
		// TODO: the reason for AddNetwork(network, position) is for UpdateNetwork(), but the latter seems unneeded
		// leaving this for now, can delete later.
		private void AddNetwork (Network network) {
			AddNetwork (network, new PointD (rng.Next (200), rng.Next (200)));
		}

		private void AddNetwork (Network network, PointD position)
		{
			lock (this) {
				if (debug) Console.Out.WriteLine ("*** Adding network {0}.", network.NetworkName);
				...
				network.NewIncomingConnection +=
					delegate (Network eNetwork, LocalNodeConnection connection)
					{
				...
				network.ConnectionUp +=
					delegate (INodeConnection connection)
					{
						Gtk.Application.Invoke (delegate {
							this.QueueDraw ();
						});
		...
		{
			lock (this) {
				if (debug) Console.Out.WriteLine ("*** Deleting network {0}.", network.NetworkName);
		...
		/* oops, doesn't look like there's an event for changes to a network.
		 * commenting this out for now.
		 *
		private void UpdateNetwork (Network network)
		{
			lock (this) {
				if (debug) Console.Out.WriteLine ("*** Updating network {0}.", network.NetworkName);
				
				PointD position = (PointD) network.Properties["position"];
				DeleteNetwork (network);
				AddNetwork (network, position);

				if (debug) Console.Out.WriteLine ("### Update network complete for {0}.", network.NetworkName);
			}
		}
		*/
		#endregion
		...
		private void AddNode (Node node, Network network) {
			...
			AddNode (node, network, new PointD (networkRect.X + rng.Next (75),
			                                    networkRect.Y + rng.Next (75)));
		}

		private void AddNode (Node node, Network network, PointD position)
		{
			lock (network) {	
				if (debug) Console.Out.WriteLine ("*** Adding node {0} to network {1}...", node.NickName, network.NetworkName);
				...
				if (nodegroups.ContainsKey (nodeIP)) {
					...
					if (debug) Console.Out.WriteLine (" -> Added node {0} to network {1} / nodegroup {2}", node.NickName, network.NetworkName, ng.Name);
				} else {
					NodeGroup ng = new NodeGroup (nodeIP, position);
					ng.Nodes.Add (node);
					node2nodegroup.Add (node, ng);
					nodegroups.Add (nodeIP, ng);
					if (debug) Console.Out.WriteLine (" -> Added node {0} to network {1} / new nodegroup {2}", node.NickName, network.NetworkName, ng.Name);
				}
			}
		}
		...
		{
			lock (network) {
				if (debug) Console.Out.WriteLine ("*** Deleting node {0} from network {1}.", node.NickName, network.NetworkName);
				Hashtable nodegroups = network.Properties["nodegroups"] as Hashtable;
				Hashtable node2nodegroup = network.Properties["node2nodegroup"] as Hashtable;
				...
				if (debug) Console.Out.WriteLine (" -> Found node {0} in nodegroup {1} / network {2}.", node.NickName, ng.Name, network.NetworkName);
				ng.Nodes.Remove (node);
				node2nodegroup.Remove (node);
				if (debug) Console.Out.WriteLine (" -> Deleted node {0} in nodegroup {1} / network {2}.", node.NickName, ng.Name, network.NetworkName);
				
				// If nodegroup is empty, delete that too.
				if (ng.Nodes.Count == 0) {
					if (debug) Console.Out.WriteLine (" -> Nodegroup {0} / network {1} empty, deleting.", ng.Name, network.NetworkName);
					nodegroups.Remove (ng.Name);
					if (debug) Console.Out.WriteLine (" -> Nodegroup {0} / network {1} deleted.", ng.Name, network.NetworkName);
				}
			}
		}
		...
		private void UpdateNode (Node node, Network network)
		{
			try {
				lock (network) {
					if (debug) Console.Out.WriteLine ("*** Updating node {0} from network {1}.", node.NickName, network.NetworkName);
					Hashtable node2nodegroup = network.Properties["node2nodegroup"] as Hashtable;
					...
					if (debug) Console.Out.WriteLine ("### Node update complete for node {0} from network {1}.", node.NickName, network.NetworkName);
				}
			} catch (Exception ex) {
				OnError (ex);
			}
		...
		private void base_ButtonPressEvent (object sender, Gtk.ButtonPressEventArgs args)
		{
			...
			if (debug) Console.Out.WriteLine ("Button Pressed {0}: user coords {1},{2}", eb.Button, mouseX, mouseY);
			...
			foreach (Network network in networkZOrder) {
				...
				if (Inside (thePoint, networkRect)) { 
					if (debug) Console.Out.WriteLine ("Click inside network {0}", network.NetworkName);
					...
					foreach (NodeGroup ng in nodegroups.Values) {
						...
						if (Inside (thePoint, ng.Position, ng.Dimension)) {
							if (debug) Console.Out.WriteLine ("Click inside group {0}", ng.Name);
							...
							//	                 np.Y + ngPosition.Y);
								if (Inside (thePoint, np, AvatarSizeD)) {
									firstNodeHit = n;
									if (debug) Console.Out.WriteLine ("Click inside node {0}", n);
									break; // clicked node n
								}
							}
							break; // click in ng but not any node
						}
					
					}
					break; // click in network but not any ng
				}
			}
		...
		private void base_MotionNotifyEvent (object sender, Gtk.MotionNotifyEventArgs args)
		{
			...
			if (debug) Console.Out.WriteLine ("Motion event: nodeDragging {0}, user coords {1},{2}", nodeDragging, em.X, em.Y);
			double mouseX = em.X;
			double mouseY = em.Y;
			DeviceToUser (ref mouseX, ref mouseY);
			PointD thePoint = new PointD (mouseX, mouseY);
			...
			if (!nodeDragging) {
				if (debug) Console.Out.WriteLine ("not nodedragging motion!");
				this.QueueDraw ();
				return;
			}
		...
		private SizeD CalculateNodeGroupTitleSize (NodeGroup ng, Cairo.Context gc)
		{
			gc.Save ();
		...
		{
			SizeD name = CalculateNodeGroupTitleSize (ng, gc);
			SizeD nodes = CalculateNGLayoutDimensions (ng.Nodes.Count);
			SizeD total = new SizeD ();
			total.Width = name.Width > nodes.Width ? name.Width : nodes.Width;
			total.Height = name.Height + nodes.Height;
			total.Width += 2 * Padding;
			total.Height += 4 * Padding;
			return total;
		}
		
		private void RenderNodeGroup (NodeGroup ng, Network network, Cairo.Context gc)
		{
			//if (debug) Console.Out.WriteLine ("Rendering Nodegroup {0} in network {1} at {2},{3}", ng.Name, network.NetworkName, ng.Position.X, ng.Position.Y);
			...
			gc.Clip ();
			gc.Rectangle (ng.Position.X, ng.Position.Y, size.Width, te.Height + 8);
			...
			//gc.Color = (selectedGroup == ng) ? black : lightGray;

			gc.Color = lightGray;

			double hostTextX = Math.Floor (ng.Position.X + (size.Width / 2) - (te.Width / 2));
			double hostTextY = Math.Floor (ng.Position.Y + te.Height + Padding);
			gc.MoveTo (hostTextX, hostTextY);
			gc.ShowText (ng.Name);
			
			SizeD nodesSize = CalculateNGLayoutDimensions (ng.Nodes.Count);
			switch (ng.Nodes.Count) {
				case 0:
					// shouldn't ever be here!
					break;
				case 1:
					double positionX = ng.Position.X;
					positionX += (te.Width > nodesSize.Width) ? (size.Width/2.0) - HalfAvatarDimension : Padding;
					positionX = Math.Floor (positionX);
					RenderNode (gc, (Node) ng.Nodes[0], positionX, ng.Position.Y + 3*Padding + te.Height);
					
					break;
				case 2:
					// position them side-by-side, separated by (padding) number of pixels, centered in the
					// space.
					double positionY = ng.Position.Y + 3*Padding + te.Height;
					double position1X = ng.Position.X + (ng.Dimension.Width / 2) - (Padding / 2) - AvatarDimension;
				...
				default:
					...
					//double internalAngle = GetNgonInternalAngle (ng.Nodes.Count);
					
					PointD center = new PointD();
					center.X = ng.Position.X + (size.Width/2.0);
					center.Y = ng.Position.Y + 
							(3*Padding) + te.Height +  // text area
							(nodesSize.Height/2); // middle of the node area;
					
					// DEBUG draw the center point of the nodegroup rendering area
					gc.Save ();
					gc.Rectangle (center.X - 1, center.Y-1, 2, 2);
					gc.Color = alphaRed;
					gc.Fill ();
					gc.Restore ();

					PointD[] nodeLocations = GetNgonNodePositions (center, ng.Nodes.Count);
					for (int i=0; i<ng.Nodes.Count; i++)
						RenderNode (gc, (Node) ng.Nodes[i],
							nodeLocations[i].X - HalfAvatarDimension,
							nodeLocations[i].Y - HalfAvatarDimension);
					break;
			}
			
			/*			
			double renderNodeY = ng.Position.Y + te.Height + 8 + 8;
			double renderNodeX = ng.Position.X + 4;
			for (int i=0; i<ng.Nodes.Count; i++) {
				Node n = (Node) ng.Nodes[i];
				
				RenderNode (gc, n, renderNodeX, renderNodeY);
				renderNodeY += 48;
			}
			*/
		...
		private void CalculateNetworkSize (Network network, Cairo.Context gc)
		{
			...
			Rectangle newNetworkRect = new Rectangle (networkX,
			                                          networkY - titleSize.Height,
			                                          networkWidth + 14,
								  networkHeight + 14 + titleSize.Height);
		...
		private int RenderNotice (string text, Cairo.Context gc, int yOffset)
		{
			...
			CreateRoundedRectPath(gc, leftPadding, topPadding, textWidth + 50, textHeight + (innerPadding * 2), 10);
			gc.Fill();

			// Render icon
			RenderPixbufToSurf(gc, noticeIcon, leftPadding + innerPadding, topPadding + innerPadding + ((textHeight / 2) - (noticeIcon.Height / 2)) );
		...
		private void RenderConnections(Network network, Cairo.Context gc)
		{
				foreach (INodeConnection inode in network.Connections) {
					if (debug) Console.Out.WriteLine ("drawing connection {0}", inode.ToString ());
					Node node1 = inode.NodeLocal;
					Node node2 = inode.NodeRemote;
					if (node1 == null || node2 == null)
						continue;
					if (!node1.Properties.ContainsKey("position") ||
					    !node2.Properties.ContainsKey("position"))
						continue;
		...
		private PointD[] GetNgonNodePositions (PointD center, int count)
		{
			if (count < 3)
				throw new Exception ("Valid n-gons have three or more sides.");

			PointD[] positions = new PointD[count];
			double radiansPerNode = (360.0 / (double) count) * (Math.PI / 180.0);
			if (debug) Console.Out.WriteLine ("Calculating centerpoints for nodes:");
			//XXX: This doesn't appear to do anything:
			//foreach (PointD p in positions)
			//	new PointD (center.X, center.Y - NgonSideLength);
			for (int i=1; i<count; i++) {
				Cairo.Matrix m = new Cairo.Matrix ();
				m.Rotate (i * radiansPerNode);
				double px = positions[i].X;
				double py = positions[i].Y;
				m.TransformPoint (ref px, ref py);
				positions[i].X = px;
				positions[i].Y = py;
				if (debug) Console.Out.WriteLine ("  {0}: ({1}, {2})", i, positions[i].X, positions[i].Y);
			}
			return positions;		
		}
		...
		private void RenderNode (Context gc, Node n, double x, double y)
		{
			...
			DrawRoundedRectThumb (gc,
					xBelow, yBelow,
					40, 40,
					10, 1,
					transparent, lightAlphaGray,
			...
			if (hoverNode == n) {
				...
				double textX = xBelow + 20 - (te.Width /2);
				double textY = yBelow + 20 - (te.Height / 2);
				double padding = 3;

				CreateRoundedRectPath (gc, textX - padding, textY - padding, te.Width + (padding * 2) , te.Height + (padding * 2), 20);
				gc.Color = darkGray;
				gc.Fill ();
		...
		private static SizeD CalculateNGLayoutDimensions (int numNodes)
		{
			switch (numNodes) {
				case 0:
					throw new Exception ("Cannot calculate layout for nodegroup with 0 nodes.");
				case 1:
					return new SizeD (AvatarDimension, AvatarDimension);

				case 2:
					return new SizeD ((2*AvatarDimension) + (Padding * 3), AvatarDimension);
				
				default:
					double d = GetNgonInscribedCircleDiameter (numNodes, AvatarInscribedCircleDiameter + 3);
					return new SizeD (d, d);
			}
		}
		
		private static double GetNgonInscribedCircleDiameter (double numSides, double sideLength)
		{
			if (numSides < 3)
				throw new Exception ("Valid n-gons have three or more sides.");
			return numSides / (Math.Sin (180.0 / sideLength));
		}
		
		private static double GetNgonInternalAngle (double numSides)
		{
			if (numSides < 3)
				throw new Exception ("Valid n-gons have three or more sides.");
		...
		private int GetNodeZone (PointD node1, PointD node2) {
			...
			int zone = 0;
			if (node2.X >= node1.X)
				zone = 4; // right half zones 4-7

			if (node2.Y >= node1.Y)
				zone += 2; // bottom quadrant, +2
