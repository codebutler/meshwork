...
namespace Meshwork.Common
{
	public static class Common
	{
		...
		public static string MD5(byte[] bytesIn) {
			var bytesOut = new System.Security.Cryptography.MD5CryptoServiceProvider().ComputeHash(bytesIn);
		...
		public static string MD5(string stringIn) {
			var bytesIn = System.Text.Encoding.UTF8.GetBytes(stringIn);
			var bytesOut = new System.Security.Cryptography.MD5CryptoServiceProvider().ComputeHash(bytesIn);
			var stringOut = BitConverter.ToString(bytesOut).Replace("-", string.Empty);
			return stringOut.ToLower ();
		}
		...
		{
			var nfi = new System.Globalization.CultureInfo( "en-US", false ).NumberFormat;
