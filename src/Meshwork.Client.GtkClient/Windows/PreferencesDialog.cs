...
namespace Meshwork.Client.GtkClient.Windows
{
	public class PreferencesDialog : GladeDialog
	{
		...
		Label nodeIdLabel;
		[Widget]
		Table downloadPathsTable;

		// Friends & Networks Tab
		[Widget]
		TreeView networksTreeView;

		// Connection Tab
		[Widget]
		Label tcpPortLabel;
		[Widget]
		Label firewallStatusLabel;
		[Widget]
		Label natStatusLabel;
		[Widget]
		Label internetIPLabel;
		[Widget]
		Label supportsIPv6Label;
		[Widget]
		Image firewallImage;
		[Widget]
		Image computerImage;
		...
		[Widget]
		Button redetectConnectionButton;

		// File Sharing Tab
		[Widget]
		TreeView sharedFoldersList;
		[Widget]
		FileChooserButton downloadsChooser;
		[Widget]
		FileChooserButton completedDownloadsChooser;

		// Plugins Tab
		[Widget]
		TreeView pluginsTreeView;

		// Advanced Tab
		[Widget]
		Notebook advancedNotebook;
		[Widget]
		TreeView advancedList;

		// Advanced Tab -> Auto-Connect
		[Widget]
		TreeView autoConnectList;
		[Widget]
		SpinButton autoConnectCountSpinButton;

		// Advanced Tab -> Connection
		[Widget]
		SpinButton nodePortSpinButton;
		[Widget]
		CheckButton nodePortOpenCheckButton;
		[Widget]
		Button testTcpPortButton;

		[Widget]
		ComboBox ipv6LocalInterfaceComboBox;

		[Widget]
		Table natOptionsTable;
		[Widget]
		CheckButton detectIPCheckButton;
		[Widget]
		Entry externalIPv4AddressEntry;
		[Widget]
		Button detectInternetIPButton;
		[Widget]
		Entry stunServerEntry;

		// Advanced Tab -> Apperance
		[Widget]
		CheckButton startInTrayCheckButton;

		// Advanced Tab -> File Transfer
		[Widget]
		CheckButton limitDownSpeedCheckButton;
		[Widget]
		CheckButton limitUpSpeedCheckButton;
		[Widget]
		SpinButton limitDownSpeedSpinButton;
		[Widget]
		SpinButton limitUpSpeedSpinButton;
		...
		public PreferencesDialog() : base(null, "PreferencesDialog")
		{
		    ...
		    provider.FromXmlString(settings.PrivateKey);
			nodeid = Common.Utils.SHA512Str(provider.ToXmlString(false));
			...
			nodeIdLabel.Markup = "<span font=\"monospace\">" + Common.Utils.FormatFingerprint(nodeid, 7) + "</span>";
			...
			{
				networksListStore.AppendValues(networkInfo);
			}
			...
			foreach (IDestination destination in Runtime.Core.DestinationManager.Destinations)
			{
			...
			foreach (InterfaceAddress addr in Runtime.Core.Platform.GetInterfaceAddresses())
			{
...
//			dialog.Run();
		...
		private void on_okButton_clicked(object o, EventArgs args)
		{
			...
			if (Common.Utils.IsValidIP(externalIPv4AddressEntry.Text) == false)
			{
			...
			TCPIPv4Destination destination = new TCPIPv4Destination(Runtime.Core, IPAddress.Parse(externalIPv4AddressEntry.Text), (uint)nodePortSpinButton.Value, nodePortOpenCheckButton.Active);
			...
			if (Runtime.Core.AvatarManager != null)
			    Runtime.Core.AvatarManager.UpdateMyAvatar();
